...
$categories = get_post_meta(get_the_ID(), 'mauna_blog_categories', true);
if ( get_query_var('paged') ) { $paged = get_query_var('paged'); }
elseif ( get_query_var('page') ) { $paged = get_query_var('page'); }
else { $paged = 1; }
$nav = (isset($redux['mauna_blog_navigation']) && $redux['mauna_blog_navigation'] != '') ? $redux['mauna_blog_navigation'] : 'small-dark';
?>
...
<section class="blog">  
	...
	endwhile;
		...
		if($postsQuery->max_num_pages > $paged) {
			echo '<div class="blog-load-more-wrapper margin-bottom-xl btn btn-blog-load"><a class="blog-load-more link-hover link-hover-color" href="'.esc_url(next_posts($postsQuery->max_num_pages, false )).'" data-djax-exclude="true">'.esc_html__('Load more', 'mauna').'</a></div>';
		}
?>
	</div>
