...
$categories = (isset($redux['mauna_portfolio4_categories']) && $redux['mauna_portfolio4_categories'] != '') ? $redux['mauna_portfolio4_categories'] : '';
if ( get_query_var('paged') ) { $paged = get_query_var('paged'); }
elseif ( get_query_var('page') ) { $paged = get_query_var('page'); }
else { $paged = 1; }

$maxPages = 1;
$postsPerPage = (isset($redux['mauna_portfolio4_posts_per_page']) && $redux['mauna_portfolio4_posts_per_page'] != '') ? $redux['mauna_portfolio4_posts_per_page'] : -1;

$gridSizer = (isset($redux['mauna_portfolio4_columns_count']) && $redux['mauna_portfolio4_columns_count'] != '') ? $redux['mauna_portfolio4_columns_count'] : 4;
$pageTitle = get_the_title();

$gutter = $redux['mauna_portfolio4_item_gutter'];
$gutter = intval($gutter) / 2;
$nav = (isset($redux['mauna_portfolio4_navigation']) && $redux['mauna_portfolio4_navigation'] != '') ? $redux['mauna_portfolio4_navigation'] : 'small-light';


$overlayHover = '';
if($redux['mauna_portfolio4_title_hover'] == true) {
...
<section class="portfolio4" style="padding: <?php echo esc_attr($gutter); ?>px">
	<?php mauna_get_page_navigation($nav); ?>
	<?php get_template_part('template-parts/header-overlay'); ?>
	<div class="page-bg"></div>
	<div class="portfolio4-masonry <?php echo 'grid-size-'.esc_attr($gridSizer); ?>" data-gutter="<?php echo esc_attr($gutter); ?>">
		<div class="portfolio-grid-sizer"></div>
		<?php
		if($redux['mauna_portfolio4_type'] == 'posts_gallery') {
			$query_params = array(
			...
			foreach ( $query_images->posts as $post ) {
				...
				$hoverImageStyle = '';
				$class = 'class="portfolio-item4 hover-basic"';
				if($hoverImage !== false && $hoverImage !== '') {
					$class = 'class="portfolio-item4"';
				...
				?>
					<div <?php echo $class; ?> style="padding: <?php echo esc_attr($gutter); ?>px">
						<?php
							if($redux['mauna_portfolio4_single_open'] == 'lightbox') {
						...
						<a class="<?php echo esc_attr($portfolioClass); ?>" href="<?php echo esc_url($portfolioUrl); ?>" <?php echo $portfolioRel; ?>>
							<img class="hide" src="<?php echo esc_url($image[0]); ?>" width="<?php echo esc_attr($image[1]);?>" height="<?php echo esc_attr($image[2]);?>"/>
							<span class="img-bg-1" style="background-image: url(<?php echo esc_url($image[0]); ?>)"></span>
							<?php
							if($hoverImageStyle != '') {
			...
			$portfolio_images = $redux['mauna_portfolio4_custom'];
			...
			if(isset($portfolio_images)) {
				...
				foreach ( $attachments as $attachment_id ) {
					...
					if($redux['mauna_portfolio4_caption'] == true && $attachment_meta['caption'] != '') {
						$attachment_caption = $attachment_meta['caption'];
						$attachment_caption = 'title="'.esc_attr($attachment_caption).'"';
					} else {
						$attachment_caption = '';
					...
					?>
						<div class="portfolio-item4 hover-basic" style="padding: <?php echo esc_attr($gutter); ?>px">
							<a href="<?php echo $galleryUrl; ?>" class="<?php echo esc_attr($galleryClass); ?>" <?php echo $attachment_caption; ?> >
								<img class="hide" src="<?php echo esc_url($image[0]); ?>" width="<?php echo esc_attr($image[1]);?>" height="<?php echo esc_attr($image[2]);?>" alt="<?php echo esc_attr($pageTitle); ?>">
								<span class="img-bg-1" style="background-image: url(<?php echo esc_url($image[0]); ?>)"></span>
							</a>
	...
	?>
	<?php if(isset($redux['mauna_portfolio4_nav_list']) && $redux['mauna_portfolio4_nav_list'] !== '') {
		echo '<div class="portfolio4-filters"><div class="menu-wrapper">';
		if(isset($redux['mauna_portfolio4_nav_list']) && $redux['mauna_portfolio4_nav_list'] !== '') {
    		mauna_default_menu_nav($redux['mauna_portfolio4_nav_list']);
		}
    	echo '</div></div>';
    }; ?>
</section>
